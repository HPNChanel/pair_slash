...
- Per-gate evidence: `docs/compatibility/acceptance-gates.yaml`
- Per-item evidence: `docs/compatibility/runtime-surface-matrix.yaml`
- Test steps: `docs/compatibility/runtime-verification.md`
...
1. Install to runtime skill directory (see `docs/workflows/install-guide.md`)
...
grep -ri "claude.code\|claude-code\|cursor runtime\|background daemon" packs/core/

# Check for MCP/vector/daemon
grep -ri "MCP\|vector.*database\|background daemon" packs/core/
```
...
Record results in `docs/compatibility/runtime-surface-matrix.yaml`.
