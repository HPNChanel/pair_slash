...
const WORKSPACE_BOUNDARY_ROOTS = [
  { root: ["packages", "core"], boundary: "core" },
  { root: ["packages", "runtimes", "codex"], boundary: "runtime-codex" },
...
function isBoundaryDependencyAllowed(sourceBoundary, targetBoundary) {
  ...
  if (sourceBoundary === "runtime-codex") {
    return targetBoundary === "core" || targetBoundary === "runtime-codex";
  }
  if (sourceBoundary === "runtime-copilot") {
    return targetBoundary === "core" || targetBoundary === "runtime-copilot";
  }
  return true;
}
...
function applyWorkspaceBoundaryRules({ repoRoot, target, checks }) {
  ...
  for (const workspacePackage of workspacePackages) {
    ...
    for (const dependencyName of localDependencies) {
      ...
      if (isBoundaryDependencyAllowed(workspacePackage.boundary, dependencyPackage.boundary)) {
        continue;
      }
      checks.push(
        createCheck({
          code: "LINT-BOUNDARY-001",
          result: "error",
          runtime: "shared",
          target,
          path: workspacePackage.packageJsonPath,
          message: `${workspacePackage.packageJson.name} (${workspacePackage.boundary}) must not depend on ${dependencyPackage.packageJson.name} (${dependencyPackage.boundary})`,
          remediation: "Move shared logic into packages/core or keep runtime-specific code inside its own runtime lane.",
        }),
      );
    }

    const sourceRoot = join(workspacePackage.dir, "src");
    for (const sourceFile of collectSourceFiles(sourceRoot)) {
      const sourceText = readFileSync(sourceFile, "utf8");
      for (const specifier of collectImportSpecifiers(sourceText)) {
        if (!specifier.toString().startsWith(".")) {
          continue;
        }
        const resolvedImport = resolve(dirname(sourceFile), specifier as string);
        const packageRoot = `${toPosixPath(workspacePackage.dir)}/`;
        const resolvedImportPosix = toPosixPath(resolvedImport);
        if (resolvedImportPosix.toString().startsWith(packageRoot) || !resolvedImportPosix.includes("/packages/")) {
          continue;
        }
        checks.push(
          createCheck({
            code: "LINT-BOUNDARY-002",
            result: "error",
            runtime: "shared",
            target,
            path: sourceFile,
            message: `${workspacePackage.packageJson.name} source must not reach across package boundaries via relative import ${specifier}`,
            remediation: "Use the public package export or move the shared code into the owning package.",
          }),
        );
      }
    }
  ...
  if (missing.length > 0) {
    checks.push(
      createCheck({
        ...
        target,
        path: entry.manifestPath,
        message: `missing file references: ${unique(missing).sort((a: any, b: any) => a.localeCompare(b)).join(", ")}`,
