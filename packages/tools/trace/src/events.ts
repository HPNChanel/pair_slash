...
import { inferFailureDomain, inferSeverity } from "./classify.ts";
import { createCorrelationId, createEventId, createSessionId, createWorkflowId } from "./ids.ts";
import { pruneTraceStore } from "./retention.ts";
import { appendTraceEvent } from "./store.ts";
...
export function emitFailureEvent(context: any, error: any, details: any = {}) {
  return emitTraceEvent(context, {
    ...
    sourceModule: details.sourceModule ?? "bin/pairslash.ts",
    payload: {
