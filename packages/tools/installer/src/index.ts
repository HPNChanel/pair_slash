...
import { getRuntimeAdapter, detectRuntimeSelection, satisfiesRuntimeRange } from "./runtime.ts";
import {
...
const MUTATING_OPERATION_KINDS = new Set(["create", "replace", "remove"]);
const CONFIG_MUTATION_SURFACES = new Set(["config", "context", "metadata", "hook", "mcp", "agent"]);
const RISKY_MUTATION_SURFACES = new Set(["config", "metadata", "hook", "mcp", "agent"]);
...
function toChangeKind(operationKind) {
...
function buildAssetDiff({ operations, runtime }) {
...
function buildPolicySummary({ lintReport, errors }) {
