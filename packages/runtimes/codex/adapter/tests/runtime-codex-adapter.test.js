...
test("codex adapter blocks invalid authoritative write from read workflow", async () => {
  ...
  assert.ok(
    result.blocking_errors.some(
      (entry) => entry.code === RUNTIME_CODEX_ADAPTER_ERROR_CODES.POLICY_BLOCKED,
    ),
  );
  assert.ok(result.policy_verdict.reasons.some((reason) => reason.code === "POLICY-HIDDEN-WRITE-BLOCKED"));
});

test("codex adapter surfaces unsupported path explicitly without fallback", async () => {
  const manifest = structuredClone(loadManifest("pairslash-plan"));
  manifest.runtime_bindings.codex_cli.compatibility.direct_invocation = "blocked";
  const result = await enforceWorkflow({
    manifest,
    request: {
      action: "run",
  ...
  assert.equal(
    ...
    RUNTIME_CODEX_ADAPTER_ERROR_CODES.CONTRACT_BUILD_FAILED,
  );
  assert.equal(result.blocking_errors[0].details.source_error_code, "PCE-CAPABILITY-001");
});

test("codex adapter keeps read-only workflow read-only and retains /skills as canonical path", async () => {
  const manifest = loadManifest("pairslash-plan");
  const result = await enforceWorkflow({
    manifest,
    request: readJson("read-only-preview-request.json"),
  });
  assert.equal(result.status, "allow");
  assert.equal(result.canonical_entrypoint, "/skills");
  assert.equal(result.selected_launch_path, "/skills");
  assert.deepEqual(listSupportedTriggerSurfaces({ manifest }), ["canonical_skill", "direct_invocation"]);
});

test("codex adapter blocks write-authority workflow when preview is missing", async () => {
  ...
  assert.ok(
    result.blocking_errors.some(
      (entry) => entry.code === RUNTIME_CODEX_ADAPTER_ERROR_CODES.PREVIEW_REQUIRED,
    ),
  );
