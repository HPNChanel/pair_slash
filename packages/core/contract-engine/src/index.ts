import {
  ...
  SUPPORTED_RUNTIMES,
  SUPPORTED_TARGETS,
  getReadWorkflowProfile as getSharedReadWorkflowProfile,
...
import { CONTRACT_ENGINE_ERROR_CODES } from "./error-codes.ts";
import { ContractEngineError, ensure } from "./errors.ts";
import { normalizeContractEnvelopeShape } from "./normalize.ts";
...
const RUNTIME_BOUNDARY_DESCRIPTORS = {
  codex_cli: {
    ...
    differences: [
      ...
      "direct-invocation-prefix:$",
      "install-surfaces:canonical_skill,support_doc,metadata,context,config,mcp",
      "trigger-surfaces:canonical_skill,direct_invocation",
  ...
  copilot_cli: {
    ...
    differences: [
      "slash-entrypoint:/skills",
      "direct-invocation-prefix:/",
      "install-surfaces:canonical_skill,support_doc,metadata,agent,hook,mcp",
      "trigger-surfaces:canonical_skill,direct_invocation,hook",
...
function buildMemoryPaths(manifest) {
  if (!isWriteAuthority(manifest)) {
    ...
    return {
      read_paths: listReadWorkflowPaths(profile, { includeFallback: true }),
      write_paths: [],
      promote_paths: [],
    };
  }
...
function buildInputContract(manifest) {
  if (isWriteAuthority(manifest)) {
    return {
      ...
      schema_refs: [
        `packages/core/spec-core/schemas/memory-write-request.schema.yaml@${MEMORY_WRITE_REQUEST_SCHEMA_VERSION}`,
        "packages/core/spec-core/schemas/memory-record.schema.yaml@0.1.0",
      ],
      validation_hints: {
        schema_refs: [
          `packages/core/spec-core/schemas/memory-write-request.schema.yaml@${MEMORY_WRITE_REQUEST_SCHEMA_VERSION}`,
...
function buildRuntimeBoundary(_manifest, runtime) {
  ...
  return {
    ...
    differences: descriptor.differences,
  };
}
...
export function buildMemoryWriteContract({ manifest, runtime, target = "repo" }: any = {}) {
