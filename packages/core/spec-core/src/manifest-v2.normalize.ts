...
function sortStrings(values: any): any {
  return [...new Set(values.filter((value: any) => typeof value === "string" && value.trim() !== ""))].sort((left: any, right: any) =>
...
function deriveCatalog(record, packName, releaseChannel, status) {
  ...
  return {
    ...
    docs_visibility:
      isOneOf(docsVisibility, PACK_DOCS_VISIBILITY) ? docsVisibility : "public",
    default_discovery:
    ...
    release_visibility:
      isOneOf(releaseVisibility, PACK_RELEASE_VISIBILITY) ? releaseVisibility : "public",
    deprecation_status:
      isOneOf(deprecationStatus, PACK_DEPRECATION_STATUSES) ? deprecationStatus : "active",
...
function deriveSupport(record, packName, runtimeBindings, memoryPermissions) {
  ...
  return {
    publisher: {
      ...
      publisher_class:
        isOneOf((pickFirstString(publisher.publisher_class) ?? defaultPublisherClass), PACK_PUBLISHER_CLASSES)
    ...
    tier_claim: isOneOf(tierClaim, PACK_TRUST_TIERS) ? tierClaim : "first-party-official",
    support_level_claim:
      isOneOf(supportLevelClaim, PACK_SUPPORT_LEVELS) ? supportLevelClaim : "official-preview",
    workflow_maturity:
      isOneOf(workflowMaturity, WORKFLOW_MATURITY_LEVELS) ? workflowMaturity : "canary",
    signature: {
      required:
    ...
    runtime_support: Object.fromEntries(
      SUPPORTED_RUNTIMES.map((runtime) => {
        ...
        return [
          ...
          {
            status:
              isOneOf(runtimeRecord.status, PACK_RUNTIME_SUPPORT_STATUSES)
                ? runtimeRecord.status
                : "unverified",
            evidence_ref: runtimeRecord.evidence_ref,
            evidence_kind:
              isOneOf(runtimeRecord.evidence_kind, PACK_RUNTIME_EVIDENCE_KINDS)
    ...
    workflow_transition: {
      from:
        support.workflow_transition?.from == null
          ? isOneOf(workflowMaturity, WORKFLOW_MATURITY_LEVELS)
            ? workflowMaturity
            : "canary"
...
function toCanonicalManifest(record, { preferCanonicalOverride }: any = {}) {
  ...
  const canonical = {
    ...
    summary,
    category,
    workflow_class: isOneOf(workflowClass, WORKFLOW_CLASSES) ? workflowClass : "read-oriented",
    phase,
